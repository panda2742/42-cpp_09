...
class SortableDeque : public ASortable<std::deque>
{
	...
	private:
		...
		void	__JacobsthalInsert(seq_t & res, seq_t & seq_to_insert);
		void	__GenerateJacobsthalIndices(seq_t & indices, uint64_t n);
		it_t	__LowerBound(it_t first, it_t last, uint64_t value);
};
